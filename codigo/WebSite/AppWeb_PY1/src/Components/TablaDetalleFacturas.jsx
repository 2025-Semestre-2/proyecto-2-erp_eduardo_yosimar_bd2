...
export default function TablaDetalleFactura({ detalleFactura }) {
    ...
    const totalPaginas = Math.ceil(detalleFactura.length / elementosPorPagina);
    const inicio = (paginaActual - 1) * elementosPorPagina;
    const facturasPagina = detalleFactura.slice(inicio, inicio + elementosPorPagina);
    ...
    const detalleProducto = async (id) => {
        ...
        //  console.log("Detalles del cliente en la tabla del detalle:", datos);
        ...
        <div className="bg-white border-emerald-200 shadow-md rounded-md overflow-x-auto">
            ...
            <table className="min-w-full divide-y divide-gray-500">
                ...
                <thead className="bg-gray-400">
                    ...
                    <th className="px-4 py-2 text-left text-sm font-semibold text-gray-700">Nombre producto</th>
                    <th className="px-4 py-2 text-left text-sm font-semibold text-gray-700">Cantidad</th>
                    <th className="px-4 py-2 text-left text-sm font-semibold text-gray-700">Precio unitario</th>
                    <th className="px-4 py-2 text-left text-sm font-semibold text-gray-700">Impuesto</th>
                    <th className="px-4 py-2 text-left text-sm font-semibold text-gray-700">Impuesto total</th>
                    <th className="px-4 py-2 text-left text-sm font-semibold text-gray-700">Total</th>
                    <th className="px-4 py-2 text-left text-sm font-semibold text-gray-700">Ganancia</th>
                ...
                <tbody className="divide-y divide-gray-200">
                    ...
                    {facturasPagina.map((factura) => (
                        <tr key={factura.ProductID}>
                            <td className="px-4 py-2 text-sm text-gray-800">{factura.NombreProducto}</td>
                            <td className="px-4 py-2 text-sm text-gray-800">{factura.Cantidad}</td>
                            <td className="px-4 py-2 text-sm text-gray-800">{factura.PrecioUnitario}</td>
                            <td className="px-4 py-2 text-sm text-gray-800">{factura.TotalImpuesto}</td>
                            <td className="px-4 py-2 text-sm text-gray-800">{factura.Total}</td>
                            <td className="px-4 py-2 text-sm text-gray-800">{factura.Impuesto}</td>
                            <td className="px-4 py-2 text-sm text-gray-800">{factura.Ganancia}</td>

              
                            {/* El boton para ver el datalle de una factura mediante su id */}
                            <td className="px-4 py-2">
                                <button
                                    onClick={() => detalleProducto(factura.ProductID)}
